...
def collapse(ui, repo, **opts):
    ...
    CAUTION: there a lots of new options, ui needs a rework so expect
    incompatible changes to the options in the future.
    """
...
def makecollapsed(ui, repo, parent, revs, branch, tagsmap, parent_hgtags, 
    ...
    ui.debug(_('makecollapsed newrev %r ctx %r node %r') 
             % (newrev, ctx, ctx.node()))
    newhex = hex(ctx.node())
    for n in nodelist:
        tagsmap[n] = newhex
...
def movedescendants(ui, repo, collapsed, tomove, movemap, tagsmap, 
    ...
    for r in sorted_tomove:
        ...
        if repo[r].files() == ['.hgtags'] and len(parents) == 1:
            ...
            ui.debug(_('movedescendants newrev %r ctx %r node %r') 
                     % (newrev, ctx, ctx.node()))
            newhex = hex(ctx.node())
...
def fix_hgtags(ui, repo, head_hgtags, tagsmap):
    # ui.debug('fix_hgtags: head_hgtags: %s' % (head_hgtags))
    # ui.debug('fix_hgtags: tagsmap: %s' % (tagsmap))
    for old in iter(head_hgtags):
        new = map_recursive(tagsmap, old)
        recreaterev(ui, repo, repo[new].rev())
        tfile = open('.hgtags', 'w')
        ...
        if '.hgtags' not in wctx:
            wctx.add(['.hgtags'])
        repo.commit(text="collapse tag fix")
...
"collapse":
        ...
        ('a', 'auto', False, _('auto collapse mode')),
        ('', 'usefirst', False, _('use first match instead of last '
                                  '(slower with --repeat)')),
        ('',  'repeat', False, _('reapeat auto collapse until no '
                                 'matches are found')),
        ('t', 'timedelta', '', _('maximum time gap between '
                                    'changesets in seconds')),
        ('u', 'userchange', False, _('don\'t auto-collapse '
                                  'when the user changed')),
        ('F', 'singlefile', False, _('don\'t autocollapse multiple '
                                     'changes to the same file')),
        ('T', 'tagcollapse', False, _('auto collapse tagged revisions')),
        ('C', 'changelog', False, _('alternate style for message combination')),
        ('L', 'movelog', '', _('appends "{move,coll} oldnode -> newnode to '
                               'this file. '
